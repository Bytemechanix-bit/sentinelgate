"""Policy model, validation, conflict detection, and nftables rendering."""
...
class Policy:
    ...
    def from_yaml(cls, path: str | Path) -> Policy:
        ...
        return cls(name=str(raw.get("name", "unnamed-policy")), rules=rules,
    ...
    def to_nftables(self, table: str = "inet sentinelgate", include_flush: bool = True) -> str:
        self.validate()
        lines = [f"# Generated by SentinelGate — policy: {self.name}", f"# Default action: {self.default_action.value.upper()}"]
        if include_flush:
            lines.append("flush ruleset")
        lines += [f"table {table} {{", "    chain input {", "        type filter hook input priority 0; policy drop;", "", "        ct state invalid drop", "        ct state established,related accept", "        iif lo accept", ""]
        for rule in self.ordered_rules():
            lines.append(f"        # [{rule.priority}] {rule.name} — {rule.comment}")
            lines.append(f"        {self._rule_to_nft(rule)}")
        lines += ["", '        log prefix "SentinelGate-DROP: " drop', "    }", "}"]
        return "\n".join(lines)
